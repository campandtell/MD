...
Usage
-----
alldci.py --pdb PDBFILE --hess HESSFILE --alldci
...
*    DFI: -dfianalysis.csv    
*    DCI: _all-dci.csv
* pctDCI: _all-pctdci.csv 

Example
-------

DFI
alldci.py --pdb 1btl.pdb

DCI
alldci.py --pdb 1btl.pdb --dci A26

Full DCI matrices
alldci.py --pdb 1btl.pdb --alldci
...
def parseCommandLine(argv):
    ...
    alldci = "--alldci" in argv
    return pdbfile, pdbid, mdhess, ls_reschain, chain_name, alldci
...
def calc_all_dci(nrmlperturbMat, ATOMS):
    """Calculate all-vs-all DCI and pctDCI matrices"""
    numres = len(ATOMS)
    dci_matrix = np.zeros((numres, numres))
    
    # Vectorized calculation of DCI
    for i in range(numres):
        dcitop = nrmlperturbMat[:, i]
        dcibot = np.sum(nrmlperturbMat, axis=1) / numres
        dci_matrix[:, i] = dcitop / dcibot
    
    # Calculate pctDCI
    pctdci_matrix = np.zeros_like(dci_matrix)
    for i in range(numres):
        pctdci_matrix[:, i] = pctrank(dci_matrix[:, i])
    
    return dci_matrix, pctdci_matrix

def output_all_dci_matrices(ATOMS, dci_matrix, pctdci_matrix, pdbid):
    """Output all-vs-all DCI and pctDCI matrices to CSV files"""
    resids = [f"{atom.chainID}{atom.res_index.strip()}" for atom in ATOMS]
    df_dci = pd.DataFrame(dci_matrix, index=resids, columns=resids)
    df_pctdci = pd.DataFrame(pctdci_matrix, index=resids, columns=resids)
    
    df_dci.to_csv(f"{pdbid}-all-dci.csv")
    df_pctdci.to_csv(f"{pdbid}-all-pctdci.csv")

def calc_dfi(pdbfile, pdbid=None, covar=None, ls_reschain=[], chain_name=None, Verbose=False,
             writetofile=False, colorpdb=False, dfianalfile=None, alldci=False):
    ...
    # output to dataframe
        # all-vs-all DCI
    if alldci:
        dci_matrix, pctdci_matrix = calc_all_dci(nrmlperturbMat, ATOMS)
        output_all_dci_matrices(ATOMS, dci_matrix, pctdci_matrix, pdbid)
...
if __name__ == "__main__":
    pdbfile, pdbid, covar, ls_reschain, chain_name, alldci = parseCommandLine(sys.argv)
    print(f"Processing {pdbfile}")
    df_dfi = calc_dfi(pdbfile, pdbid, covar=covar, ls_reschain=ls_reschain, chain_name=chain_name,
                      writetofile=True, colorpdb=True, alldci=alldci)
